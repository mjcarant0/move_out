...
class PendingView(Frame):
    def __init__(self, parent, pickup, dropoff, vehicle, price):
        ...
        tag_label = Label(lpink_box, text="PENDING", bg="#FB6F92", fg="white", font=self.tag_font, padx=8, pady=2)
        ...
        Label(pickup_frame, text="Pickup Location", bg="#FFE5EC", fg="black", font=self.label_font).pack(side=LEFT)

        Label(lpink_box, text=pickup, wraplength=300, justify=LEFT, bg="#FFE5EC", font=self.placeholder_font).pack(anchor="w", padx=35, pady=(5, 10))
        ...
        Label(dropoff_frame, text="Drop-off Location", bg="#FFE5EC", fg="black", font=self.label_font).pack(side=LEFT)

        Label(lpink_box, text=dropoff, wraplength=300, justify=LEFT, bg="#FFE5EC", font=self.placeholder_font).pack(anchor="w", padx=35, pady=(5, 10))

        dpink_box = Frame(self, bg="#FB6F92", width=367, height=55)
        dpink_box.pack(pady=(0, 0))
    ...
    # New method to retrieve current ride info
    ...
    def clear_if_canceled(self, ride_active):
        if not ride_active:
            self.destroy()
